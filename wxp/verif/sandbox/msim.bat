...
rem set TB_TOP=syn_fgyrus_tb_top
...
vlog -incr -f dgn.list +define+SIMULATION +define+USE_GPU_LF_CNTRLR -sv -incr -timescale "1ns / 10ps"  -l  compile.dgn.log
echo  Compiling TB
vlog -f verif.list +define+SIMULATION +define+USE_GPU_LF_CNTRLR -sv -incr -timescale "1ns / 10ps"  -l  compile.verif.log
...
vsim -c -novopt +OVM_TESTNAME=%TEST_NAME% -sv_lib syn_dpi_lib %TB_TOP% +define+SIMULATION +define+USE_GPU_LF_CNTRLR -l transcript.txt -permit_unmatched_virtual_intf -do "add wave -r /*;run -all"
